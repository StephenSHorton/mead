...
**DXMT_MENU_FIX** (default on, `scripts/build-wine-macos.sh`): also builds a **patched DXMT v0.80** from source (3Shain/dxmt @ v0.80 + `scripts/dxmt/wc3-menu.patch`, static LLVM 15) and installs it into the wine *builtin* tree so 64-bit games actually load DXMT for d3d11/dxgi/d3d10core — this is what paints the **Warcraft III Reforged in-game menu** (BlizzardBrowser composites it via a cross-process D3D11 shared texture D3DMetal can't provide). The patch is a small Windows-like adapter LUID (BlizzardBrowser's `--gpuluid` parse overflowed DXMT's huge `bswap(registryID)` LUID) + a no-op `IDXGIKeyedMutex`. **This switches the 64-bit d3d11/d3d10/dxgi backend from D3DMetal to DXMT** (d3d12 stays D3DMetal; the i386 launcher is untouched). Adds ~30 min on first build (LLVM 15, cached after); set `DXMT_MENU_FIX=0` to keep the stock all-D3DMetal backend.
